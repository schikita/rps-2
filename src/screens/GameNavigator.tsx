...
export const GameNavigator: React.FC<GameNavigatorProps> = ({ user, onLogout }) => {
  ...
  // --- 1. ДИНАМИЧЕСКИЙ КЛЮЧ СОХРАНЕНИЯ ---
  // Теперь ключ зависит от никнейма пользователя!
  const userStorageKey = `rps_save_${user.nickname}`;

  // --- 2. ЗАГРУЗКА ДАННЫХ (для конкретного игрока) ---
  
  const [balance, setBalance] = useState<number>(() => {
    const saved = localStorage.getItem(userStorageKey);
    // Если сохранения для этого ника есть - берем их, иначе даем 1000
  ...
  useEffect(() => {
    ...
    // Сохраняем в персональную ячейку игрока
